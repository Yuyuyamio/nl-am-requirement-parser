...
class DetachableSupportTests(unittest.TestCase):
    ...
    def test_shallow_base_fill_preserves_bottom_and_cannot_reach_ears(self):
        from types import SimpleNamespace
        import numpy as np
        ...
        mesh = trimesh.boolean.union([block((20,20,1.2),(0,0,.6)),
            block((2,2,30),(0,0,15)),block((8,2,.8),(3,0,1.8))],engine='manifold')
        observation = dict(cluster_id='shallow',z_min_mm=1.4,z_max_mm=1.64,
            kinds=('unsupported_extrusion_region',),xy_bounds_mm=[[2,-1],[7,1]])
        parts, plans = build_near_base_gap_fills(mesh,[SimpleNamespace(**observation)])
        self.assertTrue(parts)
        volume = trimesh.boolean.union(parts,engine='manifold')
        candidate = trimesh.boolean.union([mesh,volume],engine='manifold')
        np.testing.assert_allclose(candidate.bounds,mesh.bounds)
        self.assertGreater(volume.bounds[0,2],.6)
        self.assertLessEqual(volume.bounds[1,2],2.4)
        self.assertTrue(inspect_flat_printing_base(candidate)['base_flatness_passed'])
        self.assertEqual(candidate.body_count,1)
        self.assertFalse(plans[0]['temporary_support'])
        observation.update(z_min_mm=18,z_max_mm=20)
        self.assertEqual(build_near_base_gap_fills(mesh,[SimpleNamespace(**observation)])[0],[])

    def test_shallow_base_fill_requires_a_full_lower_anchor(self):
        from types import SimpleNamespace
        from am_print_executor.near_base_gap_fill import build_near_base_gap_fills
        mesh = block((2,2,30),(0,0,15))
        observation = SimpleNamespace(cluster_id='outside',z_min_mm=1.4,z_max_mm=1.64,
            kinds=('unsupported_extrusion_region',),xy_bounds_mm=[[5,5],[6,6]])
        self.assertEqual(build_near_base_gap_fills(mesh,[observation])[0],[])
