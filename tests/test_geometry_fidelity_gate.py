...
class GeometryFidelityTests(unittest.TestCase):
    ...
    def test_skinny_triangles_keep_resolution_and_area_without_budget_blowup(self):
        from am_print_executor.geometry_fidelity_gate import _surface_cells, SURFACE_SAMPLE_EDGE_MM
        triangles = np.array([[[0.,0,0],[30,0,0],[15,.02,0]]])
        cells = _surface_cells(triangles)
        self.assertLess(len(cells),2000)
        self.assertLessEqual(np.linalg.norm(cells-np.roll(cells,1,axis=1),axis=2).max(),SURFACE_SAMPLE_EDGE_MM)
        self.assertAlmostEqual(trimesh.triangles.area(cells).sum(),trimesh.triangles.area(triangles).sum(),places=10)

    def test_surface_sampling_coarsens_safely_instead_of_failing_cell_budget(self):
        from am_print_executor.geometry_fidelity_gate import _surface_cells_with_bounded_resolution
        triangles = np.array([[[0., 0, 0], [4, 0, 0], [0, 4, 0]]])
        with patch("am_print_executor.geometry_fidelity_gate.MAXIMUM_SURFACE_CELLS", 100):
            cells, sample_edge = _surface_cells_with_bounded_resolution(
                triangles, maximum_sample_edge_mm=1.0)
        self.assertGreater(sample_edge, .25)
        self.assertLessEqual(sample_edge, 1.0)
        self.assertLessEqual(len(cells), 100)
        self.assertLessEqual(
            np.linalg.norm(cells - np.roll(cells, 1, axis=1), axis=2).max(),
            sample_edge + 1e-12,
        )
        self.assertAlmostEqual(
            trimesh.triangles.area(cells).sum(),
            trimesh.triangles.area(triangles).sum(),
            places=10,
        )

    def test_large_retriangulated_plane_is_certified_without_sampling_overflow(self):
        from am_print_executor.geometry_fidelity_gate import _surface_displacement
        source = box((100, 100, 100), (0, 0, 50))
        # A changed tessellation must not be mistaken for a changed surface.
        candidate = source.subdivide()
        report = _surface_displacement(source, candidate)
        self.assertEqual(report['sample_count'], 0)
        self.assertLess(report['maximum_upper_bound_mm'], .00002)
        self.assertEqual(sum(c['triangle_count'] for c in report['coplanar_coverage_certificates']),
                         len(source.faces) + len(candidate.faces))

    def test_coplanar_certificate_checks_interior_not_just_vertices_or_area(self):
        from am_print_executor.geometry_fidelity_gate import _exclude_covered_coplanar_triangles
        triangle = np.array([[[0.,0,0],[10,0,0],[0,10,0]]])
        # Each corner is covered, but the center is a hole; extra remote area
        # cannot compensate for that hole in a coverage certificate.
        pieces = np.array([[[0.,0,0],[1,0,0],[0,1,0]],
                           [[10.,0,0],[9,0,0],[9,1,0]],
                           [[0.,10,0],[0,9,0],[1,9,0]],
                           [[20.,20,0],[100,20,0],[20,100,0]]])
        reference = trimesh.Trimesh(vertices=pieces.reshape((-1,3)),
                                   faces=np.arange(12).reshape((-1,3)),process=False)
        remaining, certificate = _exclude_covered_coplanar_triangles(triangle, reference)
        self.assertEqual(certificate['triangle_count'], 0)
        np.testing.assert_array_equal(remaining, triangle)

    def test_parallel_displaced_plane_is_not_certified_unchanged(self):
        from am_print_executor.geometry_fidelity_gate import _exclude_covered_coplanar_triangles
        triangle = np.array([[[0.,0,0],[10,0,0],[0,10,0]]])
        reference = trimesh.Trimesh(vertices=triangle[0] + [0,0,.01],faces=[[0,1,2]],process=False)
        remaining, certificate = _exclude_covered_coplanar_triangles(triangle, reference)
        self.assertEqual(certificate['triangle_count'], 0)
