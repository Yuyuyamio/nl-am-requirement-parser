...
2026-08-31 后续修复：先用投影后的共面参考三角形并集证明整个测量三角形被覆盖，包含其内部，避免把重新分割的原有大平面当成新表面细分。共面与投影各使用 0.00001 mm 数值容差，证书距离上界为二者的欧氏合成值；未被证明覆盖的部分仍接受原精度检查。不能以三个顶点接触或总面积相同替代整面覆盖。

逐层网格报告存在未支撑表面时，增加实际连续路径复核：只有在原来的 XY/Z 距离限制内，已有可达下层路径覆盖整条相关挤出段，才判定为网格边界误报。没有面积豁免，路径端点接触而中间悬空仍拒绝。

若检测到支撑本身孤立，原来的两轮预算内可以尝试一次网格支撑布局，主体间距从默认 3 mm 收至 2 mm（按喷嘴比例计算）。保持接触间隙、界面参数、线宽和原模型不变；仍需重新切片并通过支撑连续性与拆卸检查。该候选与局部补料候选合计不超出每轮三个候选，不能无限重试，不能回退为永久大支柱。选中的参数随 Auto Orient 和最终切片继续使用。

Windows 工程整理使用同目录短名随机临时文件，避免在长任务路径的文件名后附加后缀导致 Auto Orient 产物无法整理。读写、CLI 或几何测量未完成属于软件错误，不能作为重新生成模型的依据。
