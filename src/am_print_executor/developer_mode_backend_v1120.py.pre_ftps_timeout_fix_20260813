...
def _ftps_upload_verified(
    ...
    *,
    ...
    ftp = ftps.SessionReuseImplicitFTP_TLS(context=context, timeout=20)
    try:
        ftp.connect(ip_address, ftps.FTPS_PORT, timeout=20)
...
def _strict_idle_check(print_obj: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "remaining_time_zero": print_obj.get("mc_remaining_time") in (0, 0.0, "0"),
    }
    return {
