...
def slice_fixed_geometry(*, source: Path, directory: Path, studio_exe: Path,
    ...
    return {"artifact": artifact, "geometry": geometry, "gate": gate, "flat_base": flat_base,
            "removal": removal, "process_json": Path(process_json)}


def _connected_support_process(process: dict, nozzle_mm: float) -> dict:
    """One bounded fallback for isolated sparse support; keep contact gaps.

    A grid base with smaller spacing can maintain support through shrinking
    sections where snug sparse lines disappear and restart in midair. This
    changes only support layout/density, never model geometry, interface widths,
    separation gaps, or acceptance thresholds.
    """
    return {**process, "support_type": "normal(auto)", "support_style": "grid",
            "support_base_pattern": "rectilinear",
            "support_base_pattern_spacing": str(min(float(process["support_base_pattern_spacing"]), nozzle_mm*5))}
...
def prepare_verified_print(
    ...
    from am_print_executor.near_base_gap_fill import build_near_base_gap_fills
    ...
    support_layout_attempted = False
    ...
    for number in range(1, 4):
        ...
        if current["gate"]["status"] == "pass" and current["flat_base"]["base_flatness_passed"]:
            try:
                ...
                if not exc.geometry_rejected:
                    save_report(directory / "execution_error.json", {
                        "stage": "auto_orient", "error": str(exc),
                        "source": str(source), "source_sha256": source_hash,
                        "selected_geometry": str(current["geometry"]),
                        "selected_artifact": str(current["artifact"]), "rounds": rounds,
                    })
                    # An I/O/CLI failure is not evidence that the shape needs
                    # regeneration. Stop the software stage and retain inputs.
                    raise
        ...
        candidates = [current]
        # Keep the narrow slope-safe ramps separate from broad shallow fills.
        # Combining them can hide a valid ramp behind an unrelated Boolean
        # containment failure, while the two families plus the current geometry
        # still fit the frozen three-candidates-per-round budget.
        ...
        repair_candidates = []
        if envelopes:
            repair_candidates.append(("local_anchored_growth", "local", envelopes, plans))
        if support_mode == "detachable":
            shallow, shallow_plans = build_near_base_gap_fills(mesh, request.blocker_clusters,
                layer_height_mm=float(process.get("layer_height", .2)))
            if shallow:
                repair_candidates.append(("near_base_gap_fill", "shallow", shallow, shallow_plans))
        for method, suffix, candidate_envelopes, candidate_plans in repair_candidates:
            volume = trimesh.boolean.union(candidate_envelopes, engine="manifold", check_volume=True)
            ...
            record["candidates"].append({"method": method, "plans": candidate_plans, "fidelity": fidelity})
            if fidelity.get("checks", {}).get("geometry_measurement_completed") is False:
                save_report(directory / "execution_error.json", {
                    "stage": "geometry_measurement", "source": str(source), "rounds": rounds})
                raise RuntimeError("Geometry measurement could not complete: " + str(fidelity.get("error")))
            if fidelity["status"] == "pass":
                path = directory / f"round_{number}_{suffix}.stl"
                candidate.export(path)
                checked = slice_fixed_geometry(source=path, directory=directory / f"round_{number}_{suffix}_slice", **profiles)
        ...
        if candidates[-1]["gate"]["status"] != "pass" and support_mode == "permanent":
            ...
            candidates.append(checked)
        if support_mode == "detachable" and not support_layout_attempted:
            best = min(candidates, key=lambda c: (c["gate"]["status"] != "pass",
                c["gate"].get("dangerous_layer_count", 10**9), c["gate"].get("total_bad_area_mm2", float("inf"))))
            if best["gate"].get("unanchored_support_component_count", 0) > 0:
                support_layout_attempted = True
                alternate_file = directory / f"round_{number}_connected_support_process.json"
                alternate_process = _connected_support_process(process, nozzle)
                save_report(alternate_file, alternate_process)
                alternate_profiles = {**profiles, "process_json": alternate_file}
                checked = slice_fixed_geometry(source=best["geometry"],
                    directory=directory / f"round_{number}_connected_support_slice", **alternate_profiles)
                record["candidates"].append({"method": "connected_detachable_support_layout",
                    "model_geometry_changed": False, "contact_gaps_unchanged": True,
                    "process_json": str(alternate_file), "gate_status": checked["gate"]["status"],
                    "removal_status": (checked.get("removal") or {}).get("status")})
                if (checked.get("removal") or {}).get("status") == "pass":
                    candidates.append(checked)
        current = min(candidates, key=lambda c: (c["gate"]["status"] != "pass",
                      c["gate"].get("dangerous_layer_count", 10**9), c["gate"].get("total_bad_area_mm2", float("inf"))))
        if current.get("process_json"):
            # Auto Orient and its final slice must use the selected layout,
            # rather than silently returning to the failing initial profile.
            profiles["process_json"] = current["process_json"]
            process = json.loads(Path(current["process_json"]).read_text(encoding="utf-8"))
