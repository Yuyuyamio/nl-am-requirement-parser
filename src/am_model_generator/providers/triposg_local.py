...
class TripoSGLocalProvider(GenerationProvider):
    ...
    def submit(
        ...
        return self._complete_generated_model(
            request, reference_worker=t2i_result.payload,
            model_worker=triposg_result.payload,
        )

    def recover_local_submission(self, request: CreativeGenerationRequest) -> ProviderSubmission | None:
        """Finish an interrupted local postprocess without calling any worker.

        The submission layer first verifies the persisted request is identical.
        Missing raw inputs are not regenerated by recovery. Remote providers do
        not implement this capability and retain their unknown-outcome block.
        """
        self.preflight_submit(request)
        directory = self._job_windows_directory(self._provider_job_id(request.idempotency_key))
        raw = directory / "generated_model.glb"
        reference = directory / "reference_image.png"
        if not all(path.is_file() and path.stat().st_size > 0 for path in (raw, reference)):
            return None
        return self._complete_generated_model(
            request, reference_worker={"reused_existing": True, "worker_executed": False},
            model_worker={"reused_existing": True, "worker_executed": False,
                          "raw_sha256": hashlib.sha256(raw.read_bytes()).hexdigest()},
            recovered=True,
        )

    def _complete_generated_model(
        self, request: CreativeGenerationRequest, *, reference_worker: dict,
        model_worker: dict, recovered: bool = False,
    ) -> ProviderSubmission:
        provider_job_id = self._provider_job_id(request.idempotency_key)
        directory = self._job_windows_directory(provider_job_id)
        model_windows_path = directory / "generated_model.glb"
        physical_model_windows_path = directory / _PHYSICAL_MODEL_FILENAME
        reference_windows_path = directory / "reference_image.png"
        ...
        try:
            ...
            # The size exporter verifies a candidate before atomic replacement.
            # A failure must preserve any previously verified cached artifact.
            details = {
                "source_path": str(
                    model_windows_path
                ),
        ...
        submission = ProviderSubmission(
            ...
            status="completed",
            reused_existing_submission=recovered,
            ...
            provider_metadata={
                ...
                "triposg_worker": model_worker,
                "recovered_local_postprocess": recovered,
                "target_height_mm": (
                    request.target_height_mm
                ),
