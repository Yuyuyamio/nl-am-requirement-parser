...
def main():
    ...
    for name in samples:
        ...
        if record["topology"]["status"] != "pass":
            ...
            if name == "real_current_mouse":
                artifact = frozen_mouse.parent / "resume_support_30/candidate_normal_auto_30.gcode.3mf"
    ...
    rows = report["samples"]
    report["frozen_mouse_matches_validation_source"] = sha(frozen_mouse) == rows["real_current_mouse"]["source_sha_before"]
    ...
    accept["frozen_mouse_is_exact_source"] = report["frozen_mouse_matches_validation_source"]
