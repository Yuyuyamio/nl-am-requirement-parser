...
from am_print_executor.slice_geometry_frame import gate_report_in_geometry_frame, placed_mesh_from_project, verify_source_matches_slice
...
def load_accepted(directory):
    ...
    # Render the actual editable project the user opens, not a substitute mesh.
    mesh = placed_mesh_from_project(Path(receipt['project']))
    verify_source_matches_slice(mesh,Path(receipt['artifact']))
    cumulative = np.eye(4)
...
def main():
    ...
    p.add_argument('--coupon',type=Path,help='Explicitly include a calibration sample; omitted in primary delivery previews')
    args = p.parse_args()
    model = load_accepted(args.directory)
    plt.rcParams['font.family']='Microsoft YaHei'
    fig = plt.figure(figsize=(12,11) if args.coupon else (15,6.4),facecolor='#f4f6f8')
    views = [(model,12,-65,'成品外观：坐姿和平底，耳下没有永久立柱',False),
        (model,0,-90,'实际切片 · 正面：支撑从下方托住耳朵',True),
        (model,0,0,'实际切片 · 侧面：支撑与成品分开',True)]
    if args.coupon:
        views.append((load_accepted(args.coupon),12,-65,'测试小样（非小鼠成品）',True))
    for i,view in enumerate(views,1):
        ax=fig.add_subplot(2 if args.coupon else 1,2 if args.coupon else 3,i,projection='3d',facecolor='#f4f6f8')
        draw(ax,*view)
    gap = model['receipt']['removal']['minimum_measured_contact_gap_mm']
    width = max(model['widths'])
    fig.suptitle('完整小鼠模型 · 从实际交付的 3MF 工程读取',fontsize=17,color='#244758')
    ...
    path=args.directory/('detachable_supports.png' if args.coupon else 'primary_model_preview.png')
