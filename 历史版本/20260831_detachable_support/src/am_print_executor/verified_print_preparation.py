...
def finalize_verified_geometry(*, source: Path, prior_artifact: Path, directory: Path,
    ...
    prior_gate = inspect_final_gcode_printability(prior_artifact, geometry_path=source, credit_removable_support=False)
    ...
    receipt = {"status": "pass" if accepted else "blocked", "source": str(source),
               ...
               "model_self_support_required": True,
...
def prepare_verified_print(
    ...
    Visible foundations/gussets and solid infill are the generation default.
    Precision designs can disable changes; unsupported results fail closed.
    ...
    result = {"status": "slice_complete" if accepted else "blocked", "pipeline": "model_self_support_orient_reslice_v2",
