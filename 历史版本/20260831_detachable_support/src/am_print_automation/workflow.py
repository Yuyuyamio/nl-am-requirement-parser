...
class AutomationConfig:
    ...
    # Print-ready generation includes reported permanent model supports.
    # Precision parts can explicitly disable redesign and fail closed.
...
def _validate_prepared_slice(services, prepared, artifact, geometry):
    ...
    if prepared.get("pipeline"):
        ...
        valid=(prepared.get("status")=="slice_complete"
               and prepared.get("model_self_support_required") is True
               and receipt.get("status")=="pass"
               and receipt.get("model_self_support_required") is True
...
class ProductionServices:
    ...
    def validate_gcode(
        ...
        result = inspect_final_gcode_printability(
            ...
            credit_removable_support=False,
    ...
    def slice_stl_with_support(
        ...
        for parent in list(Path(stl_path).resolve().parents)[:4]:
            ...
            if (previous.get("pipeline")=="model_self_support_orient_reslice_v2"
