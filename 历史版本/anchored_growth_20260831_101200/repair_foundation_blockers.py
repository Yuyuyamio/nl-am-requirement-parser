...
def main():
    ...
    for strength in ("minimal", "balanced", "strong"):
        selected = [p for p in plans if p.strength == strength and p.status == "candidate"]
        if not selected:
            continue
        envelopes = [build_envelope_mesh(plan) for plan in selected]
        ...
        fidelity = inspect_geometry_fidelity(source=source, candidate=candidate, repair_volume=repair_volume,
            budget=request.repair_budget, protected_base_clearance_z_mm=max(p.protected_base_clearance_z_mm for p in selected))
        record = {"strength": strength, "plans": [p.to_dict() for p in selected], "fidelity": fidelity}
        ...
        print(strength, len(selected), fidelity["status"], fidelity["blockers"], flush=True)
