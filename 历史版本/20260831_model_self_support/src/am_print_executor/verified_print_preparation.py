    ...
    for number in range(1, 4):
        ...
        if candidates[-1]["gate"]["status"] != "pass":
            try:
                ...
                candidate, action = add_integrated_support_ribs(mesh, request.blocker_clusters, margin_mm=.8,
                                                                critical_regions=tuple(placed_critical))
                ...
                candidate, action = add_integrated_support_ribs(foundation, request.blocker_clusters, margin_mm=.8,
                                                                critical_regions=tuple(placed_critical))
