...
def build_anchored_growth_envelopes(
    ...
    slope_xy_per_z: float = .9, maximum_depth_mm: float | None = None,
    ...
    # Reach a verified integrated foundation when a thin appendage has no
    # usable local interior; still bound depth relative to overall size.
    maximum_depth_mm = float(maximum_depth_mm if maximum_depth_mm is not None else min(10.0, float(source.extents[2]) * .25))
