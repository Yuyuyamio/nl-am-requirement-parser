...
def _surface_cells(triangles: np.ndarray) -> np.ndarray:
    """Deterministic subdivision of measurement triangles, never the mesh."""
    finished = []
    count = 0
    pending = triangles
    while len(pending):
        edge = np.linalg.norm(pending - np.roll(pending, 1, axis=1), axis=2).max(axis=1)
        small = pending[edge <= SURFACE_SAMPLE_EDGE_MM]
        finished.append(small)
        count += len(small)
        large = pending[edge > SURFACE_SAMPLE_EDGE_MM]
        if count + len(large) * 4 > MAXIMUM_SURFACE_CELLS:
            raise ValueError("surface_measurement_budget_exceeded")
        a, b, c = large[:, 0], large[:, 1], large[:, 2]
        ab, bc, ca = (a + b) / 2, (b + c) / 2, (c + a) / 2
        pending = np.concatenate([np.stack(t, axis=1) for t in
...
def _surface_displacement(source: trimesh.Trimesh, candidate: trimesh.Trimesh) -> dict[str, Any]:
    distances, areas, bounds = [], [], []
    sample_count = 0
    for mesh, reference in ((candidate, source), (source, candidate)):
        cells = _surface_cells(_changed_triangles(mesh, reference))
        if not len(cells):
            continue
        ...
        changed = distance > NUMERICAL_DISTANCE_MM
        # Unchanged surface must not dilute the percentile of the repair.
    ...
    return {"method": "bidirectional_changed_surface_area_weighted_deterministic_cells",
            "sample_edge_mm": SURFACE_SAMPLE_EDGE_MM, "sample_count": sample_count,
    ...
    *, source: trimesh.Trimesh, candidate: trimesh.Trimesh,
    ...
    try:
        ...
        if all(checks.values()):
            displacement = _surface_displacement(source, candidate)
            report["surface_displacement"] = displacement
            checks["surface_p95_within_budget"] = displacement["p95_mm"] <= budget.maximum_surface_displacement_p95_mm
