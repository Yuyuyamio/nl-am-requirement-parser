...
function statusPresentation(snapshot) {
  ...
  if (snapshot.status === "needs_geometry_regeneration") return ["模型未通过检查，已停止", "自动修复和一次重新生成后仍未通过。没有上传或开打，请修改需求后新建任务。"];
...
function renderControls(snapshot) {
  ...
  const canRetry = snapshot.terminal && !["ready_to_print", "print_started", "manual_reconciliation_required", "needs_geometry_regeneration", "awaiting_clarification"].includes(snapshot.status);
...
function renderDelivery(snapshot) {
  ...
  const key = model ? `${snapshot.job_id}:${model.sha256}` : null;
  ...
  elements.previewHint.textContent = "正在读取最终成品几何…";
  fetch(model.url, { signal: previewAbort.signal, cache: "no-store" }).then(async (response) => {
    if (!response.ok || response.headers.get("X-Artifact-SHA256") !== model.sha256) throw new Error("最终文件核验失败");
    const data = await response.arrayBuffer();
    if (previewKey !== key) return;
    elements.modelPreview.classList.remove("hidden");
    showModelPreview(elements.modelPreview, data);
    elements.previewHint.textContent = "拖动旋转。直接读取最终 STL，显示成品本体，不含可拆支撑；支撑请打开 02 切片查看。";
  }).catch((error) => {
    if (previewKey === key && error.name !== "AbortError") elements.previewHint.textContent = "预览暂不可用，请下载 01 模型在 Bambu Studio 中检查。";
  });
}
