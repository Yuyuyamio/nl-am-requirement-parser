...
class FakeServices:
    ...
    def validate_gcode(
        ...
        passed = (
            ...
            else self.support_printability_passes
        )
        return {
            "status": "pass" if passed else "blocked",
            "blockers": [] if passed else ["unsupported_model_region"],
            "resolution": (
            ...
            "feedback_to_m2": (
                ...
                if passed
                    ...
                    "reason": "unsupported layer island",
                    "dangerous_layers": [{"z_mm": 4.2}],
                }
            ),
        }
...
class TestAutomaticPrintWorkflow(unittest.TestCase):
    ...
    def test_support_reslice_block_routes_to_geometry_regeneration(self) -> None:
        services = FakeServices(
            self.root,
            initial_printability_passes=False,
        ...
        self.assertEqual(result.status, "needs_geometry_regeneration")
        self.assertEqual(secret_calls, 0)
        self.assertNotIn("printer_upload", services.calls)
        self.assertNotIn("print_start", services.calls)
        state = json.loads(Path(result.state_file).read_text(encoding="utf-8"))
        self.assertEqual(
            state["stages"]["m3_support_printability"]["status"],
            "completed",
        )
        self.assertEqual(
            state["stages"]["printer_upload"]["status"],
            "skipped",
        )
        self.assertEqual(
            state["printability_resolution"],
            "needs_geometry_regeneration",
        )
        self.assertEqual(
            state["manufacturability_feedback"]["dangerous_layers"],
            [{"z_mm": 4.2}],
        )
