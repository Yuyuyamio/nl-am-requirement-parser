...
SUPPORTED_SOURCE_MODES = {"auto", "ams", "external_spool"}
MAPPING_CAPACITY = 5
...
def _slot_matches(slot: Mapping[str, Any], requirement: Mapping[str, Any]) -> bool:
    return (
        slot.get("exists", True)
        and slot.get("material") == requirement.get("material")
        and slot.get("rgba") == requirement.get("rgba")
    )


def _unique_ams_assignment(
    requirements: list[dict[str, Any]],
    slots: list[dict[str, Any]],
) -> list[dict[str, Any]] | None:
    candidates: list[list[dict[str, Any]]] = []
    for requirement in requirements:
        matches = [slot for slot in slots if _slot_matches(slot, requirement)]
        candidates.append(matches)
    solutions: list[list[dict[str, Any]]] = []

    def search(index: int, used: set[int], current: list[dict[str, Any]]) -> None:
        if len(solutions) > 1:
            return
...
def _resolve_source(
    ...
    preference = os.environ.get(SOURCE_MODE_ENV, "auto").strip().lower() or "auto"
    if preference not in SUPPORTED_SOURCE_MODES:
        raise BambuHeadlessNativeSenderError(
            f"{SOURCE_MODE_ENV} must be one of {sorted(SUPPORTED_SOURCE_MODES)}."
        )

    ams_solution = _unique_ams_assignment(requirements, state.get("ams_slots", []))
    external = state.get("external")
    external_match = (
        len(requirements) == 1
        and isinstance(external, Mapping)
        and external.get("material") == requirements[0]["material"]
        and external.get("rgba") == requirements[0]["rgba"]
    )

    if preference == "ams":
        if ams_solution is None:
            raise BambuHeadlessNativeSenderError(
                "AMS was explicitly requested but no unique exact material/color assignment exists."
            )
        return {"source": "ams", "slots": ams_solution}

    if preference == "external_spool":
        if not external_match:
            raise BambuHeadlessNativeSenderError(
                "External spool was explicitly requested but its reported material/color does not exactly match the project."
            )
        return {"source": "external_spool", "external": dict(external)}

    # AUTO: if both are valid, prefer what the printer reports as currently loaded.
    if ams_solution is not None and external_match:
        tray_now = state.get("tray_now")
        ams_wires = {int(slot["wire_slot"]) for slot in ams_solution}
        if tray_now in ams_wires:
            return {"source": "ams", "slots": ams_solution}
        if tray_now in {254, 255}:
            return {"source": "external_spool", "external": dict(external)}
        raise BambuHeadlessNativeSenderError(
            "Both AMS and external spool exactly match the project, but the active source is ambiguous. "
            f"Set {SOURCE_MODE_ENV}=ams or external_spool explicitly."
        )
    ...
    if external_match:
        return {"source": "external_spool", "external": dict(external)}
    raise BambuHeadlessNativeSenderError(
        "No exact filament source matches the project. The native sender will not guess or default to external spool."
    )
