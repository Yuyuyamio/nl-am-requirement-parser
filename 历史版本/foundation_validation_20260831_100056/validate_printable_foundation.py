...
def main():
    ...
    process = json.loads((profiles / "process/0.20mm Standard @BBL X1C.json").read_text(encoding="utf-8-sig"))
    ...
    command = [str(studio), "--arrange", "0", "--ensure-on-bed", "--slice", "0", "--debug", "2",
               ...
               "--load-settings", ";".join([resolved["machine"], resolved["process"]]),
               "--load-filaments", ";".join(resolved["filaments"]), str(stl)]
